...
class CropAvatar {
    ...
    private function setFile($file) {
        ...
        if ($errorCode === UPLOAD_ERR_OK) {
            ...
            if ($type) {
                ...
                $single_filename = date('YmdHis');
                $theFileName2017_withoutExt = $single_filename . '.original';
                $theFileName2017 = $theFileName2017_withoutExt . $extension;
                $src = 'uploaded_images/' . $theFileName2017;
                $theFileName2017 = $single_filename . ".png";
                ...
                if ($type == IMAGETYPE_GIF || $type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG) {
                    ...
                    if ($result) {
                        ...
                        $idToCheck2017 = $_GET["theidToFind"];
            ...
            ogenius_nds_db_normal_users_avatar='{$theFileName2017}' WHERE ogenius_nds_db_normal_users_id='{$idToCheck2017}'";

                        if ($resQuery = $database -> query($sqlNDSUpdateRegistration)) {

                            $sqlNDS_avatars_tracker = "INSERT INTO ogenius_nds_db_profile_image_imgs_src 
                            (ogenius_nds_db_profile_image_imgs_src_user_id, ogenius_nds_db_profile_image_imgs_src_imgsrc,
                            ogenius_nds_db_profile_image_imgs_src_regdate)
                            VALUES('{$idToCheck2017}','{$theFileName2017}','{$date}')";

                            if ($res_avatars_tracker = $database -> query($sqlNDS_avatars_tracker)) {
                                //--------------------find the
    ...
    private function setDst() {
        $this -> dst = 'uploaded_images/' . date('YmdHis') . '.png';
    }
