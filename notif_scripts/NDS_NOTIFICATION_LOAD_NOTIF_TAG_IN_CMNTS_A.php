...
ORDER BY ogenius_nds_db_normal_users_id DESC LIMIT 1";
    ...
    for ($i = 0; $i < sizeof($tagIdentifier); $i++) {
        for ($n = 0; $n < sizeof($tagKeyword); $n++) {
            $OverallTag[$w] = $tagIdentifier[$i] . $tagKeyword[$n];
        }
        $w++;
    }
}
...
foreach ($OverallTag as $word) {
    $sql[] = "ogenius_nds_db_community_posts_comments_comment LIKE '%" . $word . "%'";
}
...
//-------------------
$sql_AllMyPostsTags = "SELECT * FROM ogenius_nds_db_community_posts_comments
                  WHERE 
                   " . implode(" OR ", $sql) . " 
...
while ($row_AllMyPostsTags = mysqli_fetch_array($query_AllMyPostsTags)) {
    $AllMyPostsTags_posts_id_array[] = $row_AllMyPostsTags['ogenius_nds_db_community_posts_comments_id'];
    $AllMyPostsTags_poster_id_array[] = $row_AllMyPostsTags['ogenius_nds_db_community_posts_comments_commentator_id'];
...
$sql_observe_notif = "SELECT * FROM ogenius_nds_db_community_posts_comments
 LEFT JOIN ogenius_nds_db_normal_users
            ON ogenius_nds_db_community_posts_comments.ogenius_nds_db_community_posts_comments_commentator_id=ogenius_nds_db_normal_users.ogenius_nds_db_normal_users_id 
                WHERE  
                  ogenius_nds_db_community_posts_comments_id NOT IN (" . implode(',', $remoteIds_Array) . ")                 
                ...
                AND         ogenius_nds_db_community_posts_comments_id  IN (" . implode(',', $AllMyPostsTags_posts_id_array) . ") 
