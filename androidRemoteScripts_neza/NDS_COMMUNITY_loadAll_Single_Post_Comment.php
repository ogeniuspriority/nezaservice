...
$postParentId = $_POST['postParentId'];
if ($_POST['select_directive'] == "allposts") {

    //----------------find the id of the parent post id--
    $sql_Posts_Comments_parent = "SELECT *   
             FROM ogenius_nds_db_community_posts_comments            
WHERE   ogenius_nds_db_community_posts_comments_id='{$postParentId}' 
AND     ogenius_nds_db_community_posts_comments_active_status='0'  ORDER BY 
    ogenius_nds_db_community_posts_comments_regdate DESC LIMIT 1


";

    if ($query_Posts_Comments_parent = $database -> query($sql_Posts_Comments_parent)) {
        while ($res__parent = mysqli_fetch_array($query_Posts_Comments_parent)) {
            $thePostIdHahaha = $res__parent['ogenius_nds_db_community_posts_comments_posts_post_id'];
...
WHERE P.ogenius_nds_db_community_posts_id='{$thePostIdHahaha}' AND P.ogenius_nds_db_community_posts_active='0'
ORDER BY P.ogenius_nds_db_community_posts_id DESC LIMIT 1";
