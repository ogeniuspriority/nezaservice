...
$sql_2017_institution_suggestions = "SELECT * FROM  ogenius_nds_db_rgb_query WHERE     ogenius_nds_db_rgb_query_id='{$suggestIonId}' ORDER BY      ogenius_nds_db_rgb_query_id DESC LIMIT 1";
if ($query_2017_institution_suggestions = $database -> query($sql_2017_institution_suggestions)) {
    ...
    while ($res_2017_institution_suggestions = mysqli_fetch_array($query_2017_institution_suggestions)) {
        $theSender_id = $res_2017_institution_suggestions['ogenius_nds_db_rgb_query_query_provider'];
        $theSuggestion_acquire_date = $res_2017_institution_suggestions['ogenius_nds_db_rgb_query_date'];
        //--------------------------------
...
<div style="color: #CDCDCD;background: #333333;padding: 10px" class="startcolstart">
    <h4>The query/question is alive for <span style="color: gold;text-shadow: 1px 1px 1px #333333;"><?php echo "".time_to_sec($theSuggestion_acquire_date); ?></span></h4>
    <h5>Details about the sender below</h5>
    <div style="">
...
while ($res_2017_institution_suggestions_user_data = mysqli_fetch_array($query_2017_institution_suggestions_user_data)) {
?>
<div style="color: #CDCDCD;background: #333333;border-radius: 6px;">
    <h4>The query/question is alive for <span style="color: gold;text-shadow: 1px 1px 1px #333333;"><?php echo "".time_to_sec($theSuggestion_acquire_date); ?></span></h4>
    <h5>Details about the sender below</h5>
    <div style="">
