...
if ($query_institutions_only = $database->query($sql_institutions_only)) {
    ...
    while ($res_institutions_only = mysqli_fetch_array($query_institutions_only)) {
        ...
        <div class="startrowstart" style="margin:3px auto;padding: 2px 4px; cursor: pointer; background-color: whitesmoke">
            <input id="<?php echo "" . $actOnThisQueryLogic; ?>"  type="radio" class="<?php echo "" . $_POST['main_page_institution_RGB_TheQueryID']; ?>" title="<?php echo "" . $res_institutions_only['ogenius_nds_db_normal_users_id']; ?>" onchange="main_page_institution_forwardOrDelegateThisQuery(this,<?php echo "" . $_POST['main_page_institution_RGB_TheQueryID']; ?>)" name="rgb_form_suggest<?php echo "" . $queryID; ?>" />
